    ...
    const fields2 = [
      ...
      },
      {
        group: {
          ...
          shadow: true,
          titleLeft() {
              return h('span', {style: 'color: red;'}, '左边插槽')
          },
          titleRight() {
              return h('span', {style: 'color: red;'}, '右边插槽')
          },
          children: [
            {
              label: '学历',
...
| enter-method        | 回车后调用的方法                                             | `(e: Event) => void`                             | -                                                   | -       |
| readonly            | 表单内容是否只读                                             | boolean                                          | -                                                   | false   |
...
| fiexd-error-message | 是否固定验证失败消息在右下角 | boolean | - | false |
| showText | 只读状态下是否只显示文本，不显示form表单组件 | boolean | - | false |
| textProps | 显示文本属性配置 | object | - | - |
...
| titleLeft  | 分组标题左侧插槽                                 | `(modelValue: T) => VNode`   | -       | -       |
| titleRight | 分组标题右侧插槽                                 | `(modelValue: T) => VNode`   | -       | -       |
